...
main:   # setup segment-registers DS, ES, FS so that we can access
        ...
        # save the default interrupt-vector for the timer (INT-0x08)

        mov     %fs:0x08*4+0, %ax       # copy vector's lo-word
        mov     %ax, sav+0              #  to our save-location
        mov     %fs:0x08*4+2, %ax       # copy vector's hi-word
        mov     %ax, sav+2              #  to our save-location

        # install a pointer to our own handler in the IVT (INT-0x08)

        cli                     # <---- ENTER 'CRITICAL SECTION'
        movw    $isr_tick, %fs:0x08*4+0 # write vector's lo-word
        movw    $seg_boot, %fs:0x08*4+2 # write vector's hi-word
...
nxpel:  lodsb                           # fetch next character-byte
        ...
        # remove that item from the keyboard's input-queue

        xor     %ah, %ah                # return keyboard entry
        int     $0x16                   # request BIOS service

        # check to see if it was the <ESCAPE>-key
        cmp     $0x011B, %ax            # did user hit <ESCAPE>?
        jne     again                   # no, continue this demo

        # restore original interrupt-vector to the IVT (INT-0x08)
        # [We can do this safely, in a single 'atomic' operation]

        mov     sav+0, %eax             # fetch the saved vector
        mov     %eax, %fs:0x08*4        # put it back in the IVT
